...
const Navbar = ({ toggleSidebar, sidebarOpen }) => {
  ...
  return (
    <nav className={`${currentTheme.primary} ${currentTheme.border} border-b px-4 py-3 flex items-center justify-between sticky top-0 z-40 shadow-sm`}>
      {/* Left Section */}
      ...
      <div className="flex-1 max-w-md mx-4 hidden md:block">
        <form onSubmit={handleSearch} className="relative">
          ...
          <input
            ...
            className={`w-full pl-10 pr-4 py-2 rounded-lg ${currentTheme.secondary} ${currentTheme.text} border ${currentTheme.border} focus:outline-none focus:ring-2 focus:ring-[#005f73] focus:border-transparent`}
          />
        </form>
      </div>
      ...
      <div className="flex items-center space-x-2">
        ...
        <div className="relative">
          <button
            ...
            className={`p-2 rounded-lg ${currentTheme.textSecondary} hover:${currentTheme.secondary} flex items-center space-x-1 transition-colors`}
            title="Change Theme"
          >
            <Palette size={20} />
            <ChevronDown size={14} />
            ...
            <div className={`absolute right-0 mt-2 w-48 ${currentTheme.card} rounded-lg shadow-lg border ${currentTheme.border} py-2 z-50`}>
              ...
              {themes.map((theme) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 hover:${currentTheme.secondary} ${currentTheme.text} capitalize flex items-center justify-between transition-colors`}
                >
        ...
        <div className="relative">
          <button 
            ...
            className={`p-2 rounded-lg ${currentTheme.textSecondary} hover:${currentTheme.secondary} relative transition-colors`}
            title="Notifications"
          >
            <Bell size={20} />
            ...
            <div className={`absolute right-0 mt-2 w-80 ${currentTheme.card} rounded-lg shadow-lg border ${currentTheme.border} py-2 z-50`}>
              ...
              <div className="max-h-64 overflow-y-auto">
                {notifications.map((notification) => (
                  <div key={notification.id} className={`px-4 py-3 hover:${currentTheme.secondary} ${notification.unread ? 'bg-[#005f73] bg-opacity-5' : ''}`}>
        ...
        {/* Messages */}
        <button className={`p-2 rounded-lg ${currentTheme.textSecondary} hover:${currentTheme.secondary} transition-colors`} title="Messages">
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center space-x-2 p-2 rounded-lg ${currentTheme.textSecondary} hover:${currentTheme.secondary} transition-colors`}
          >
          ...
          {showProfileMenu && (
            <div className={`absolute right-0 mt-2 w-56 ${currentTheme.card} rounded-lg shadow-lg border ${currentTheme.border} py-2 z-50`}>
              ...
              <button className={`w-full text-left px-4 py-2 hover:${currentTheme.secondary} ${currentTheme.text} flex items-center space-x-2 transition-colors`}>
                <User size={16} />
              ...
              <button className={`w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 flex items-center space-x-2 transition-colors`}>
                <LogOut size={16} />
                <span>Sign Out</span>
              </button>
            </div>
